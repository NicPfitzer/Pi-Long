...
def predict_tracks(
    ...
    *,
    rank_method: str = "dino",
    dino_image_size: int = 336,
    ...
    if rank_method == "dino":
        query_frame_indexes = generate_rank_by_dino(
            images, query_frame_num=query_frame_num, image_size=dino_image_size, device=device
        )
    else:
        # Uniformly sample query frames (always include frame 0)
        S = images.shape[0]
        if query_frame_num <= 1 or S <= 1:
            query_frame_indexes = [0]
        else:
            rest = max(0, query_frame_num - 1)
            if S - 1 > 0 and rest > 0:
                idxs = np.linspace(1, S - 1, num=min(rest, S - 1), dtype=int).tolist()
            else:
                idxs = []
            query_frame_indexes = [0] + idxs
...
def _forward_on_query(
    ...
    pred_track, pred_vis, _ = predict_tracks_in_chunks(
        tracker, images_feed, query_points, fmaps_feed, fine_tracking=fine_tracking, fine_chunk=fine_chunk
    )
